    ...
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header px-4 pt-4 border-0 pb-1">
                <h3 class="text-capitalize">{{translate('Layanan & Sparepart')}}</h3>
                ...
                <form action="{{route('provider.booking.service.update_booking_service')}}" method="POST"
                    ...
                    <div class="table-responsive">
                        <table class="table text-nowrap align-middle mb-0" id="service-edit-table">
                            ...
                            <thead>
                            <tr>
                                <th colspan="2" class="ps-lg-3">{{translate('Service')}}</th>
                                {{-- <th>{{translate('Price') . ' (' . currency_symbol() . ')'}}</th> --}}
                                <th colspan="2">{{translate('Qty') }}</th>
                                {{-- <th colspan="2">{{translate('Discount') . ' (' . currency_symbol() . ')'}}</th> --}}
                                {{-- <th>{{translate('Total') . ' (' . currency_symbol() . ')'}}</th> --}}
                                <th colspan="2" class="text-center">{{translate('Action')}}</th>
                            </tr>
                            </thead>
                            ...
                            @foreach($booking->detail as $key=>$detail)
                                <tr id="service-row--{{$detail?->variant_key}}">
                                    ...
                                    <td id="service-cost-{{$detail?->variant_key}}"></td>
                                    <td>
                                    ...
                                    <td id="discount-amount-{{$detail?->variant_key}}"></td>
                                    <td id="total-cost-{{$detail?->variant_key}}"></td>
                                    <td>
            ...
            <div class="modal-footer d-flex justify-content-end gap-3 border-0 pt-0 pb-4">
                ...
                <button type="submit" class="btn btn--primary"
                        form="booking-edit-table">{{translate('Simpan')}}</button>
            </div>
        </div>
    </div>
</div>
