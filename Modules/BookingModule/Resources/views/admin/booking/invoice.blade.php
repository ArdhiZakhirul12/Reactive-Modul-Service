...
<head>
    ...
    <title>{{ translate('Laporan Servis') }}</title>
            ...
            <div>
                <header>
                    <div class="row align-items-center">
                        <div class="col">
                            <h3 class="text-uppercase fw-700">{{ translate('Laporan_Servis') }}</h3>
                ...
                <div class="white-box-content border rounded-12 border">
                    ...
                    <div class="p-3">
                        <div class="row contacts">
                            ...
                            <div class="col">
                                <h6 class="fz-12">{{ translate('Teknisi') }}</h6>
                                <div class="fs-9">{{ $booking->serviceman?->user->first_name }}
                                    {{ $booking->serviceman?->user->last_name }}</div>
                            </div>
                            ...
                            <div class="col border-left">
                                ...
                                <div class="fs-9">{{ translate('Tanggal selesai') }} :
                                    @if (!is_null($booking->status_histories))
                                        @foreach ($booking->status_histories as $value)
                                            {{ date('d-M-Y h:ia', strtotime($value->updated_at)) }}
                                        @endforeach
                                    @endif
                                </div>
                            </div>
                        ...
                        <table cellspacing="0" cellpadding="0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-left text-uppercase" colspan="2">
                                        {{ translate('Machine_Type') }}</th>
                                    <th class="text-left text-uppercase">{{ translate('description') }}</th>
                                    <th class="text-center text-uppercase">{{ translate('penanganan_teknisi') }}</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($booking->detail as $index => $item)
                                    <tr>
                                        <td class="border-bottom text-left">
                                            {{ (strlen($index + 1) < 2 ? '0' : '') . $index + 1 }}
                                        </td>
                                        <td colspan="2" class="border-bottom text-left">
                                            <div>{{ $booking->subCategory->name }}</div>
                                            @if (is_null($booking->customer?->customer_machine))
                                                <div>Pelanggan tidak punya mesin</div>
                                            @else
                                                @if ($booking->customer->customer_machine->detail_customer_machine->isEmpty())
                                                    <div>Pelanggan tidak punya mesin</div>
                                                @else
                                                    @foreach ($booking->customer->customer_machine->detail_customer_machine as $customer_machine)
                                                        {{-- <div>Nomer Seri : {{ $customer_machine->keterangan }}</div> --}}
                                                        @if (is_null($customer_machine->keterangan))
                                                            <div>Nomer Seri: -</div>
                                                        @else
                                                            <div>Nomer Seri : {{ $customer_machine->keterangan }}</div>
                                                        @endif
                                        ...
                                        @if ($index == 0)
                                            <td rowspan="{{ count($booking->detail) }}" class="border-bottom text-right"
                                                style="max-width: 200px; word-wrap: break-word;);">
                                                <div class="text-center ">
                                                    {{ is_null($booking->serviceman_notes) ? '-' : $booking->serviceman_notes }}</div>
                                            </td>
                                        @endif
                                ...
                                <td class="border-bottom text-right">{{with_currency_symbol($item->total_cost - $item->tax_amount)}}</td> --}}
                                        {{-- <td class="border-bottom text-right">{{with_currency_symbol($item->total_cost)}}</td> --}}

                                        {{-- <td class="border-bottom text-center" colspan="2">{{ $subCategory->name }}</td> --}}
                        ...
                        <tr>
                            <td colspan="3"></td>
                            <td class="">{{translate('warranty_discount')}} </td>
                        ...
                        <tr>
                            <td colspan="3"></td>
                            <td class="fw-700 border-top">{{translate('Total')}}</td>
                            <td class="fw-700 border-top">{{with_currency_symbol($booking->total_booking_amount - $booking->total_tax_amount)}}</td>
                        </tr>
