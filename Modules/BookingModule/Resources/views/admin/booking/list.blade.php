                                ...
                                <form action="{{url()->current()}}?booking_status={{$queryParams['booking_status']}}"
                                    ...
                                    <div class="input-group search-form__input_group">
                                        ...
                                        <input type="search" class="theme-input-style search-form__input"
                                               ...
                                               placeholder="{{translate('Cari_ID_Pemesanan')}}">
                                    </div>
                                    <button type="submit"
                            ...
                            <div class="table-responsive">
                                <table id="example" class="table align-middle">
                                    ...
                                    <tr>
                                        ...
                                        <th>{{translate('Teknisi')}}</th>
                                        <th>{{translate('Status')}}</th>
                                        <th>{{translate('Permintaan Masuk')}}</th>
                                        <th>{{translate('Permintaan Tanggal')}}</th>
                                        <th>{{translate('Booking_Date')}}</th>
                                        <th>{{translate('Jadwal Perbaikan')}}</th>
                                        <th>{{translate('Perbaikan Selesai')}}</th>
                                    ...
                                    @foreach($bookings as $key=>$booking)
                                        <tr>
                                            ...
                                            <td>{{$booking->serviceman?->user?->first_name ?? translate('Not_Assign')}}</td>
                                            <td>
                                            ...
                                            <td>{{date('d-M-Y',strtotime($booking->service_schedule))}}</td>
                                            @else
                                            <td>{{date('d-M-Y h:ia',strtotime($booking->service_schedule))}}</td>
                                            @endif --}}
                                             @if (is_null($booking?->customRequestBooking))
                                                <td>-</td>
                                            @else
                                                <td class="text-primary">{{date('d-M-Y h:ia',strtotime($booking->customRequestBooking->customRequest->created_at))}}</td>
                                            @endif
                                            @if (is_null($booking?->customRequestBooking?->customRequest->schedule_time))
                                                <td>-</td>
                                            @else
                                                <td class="text-primary">{{date('d-M-Y h:ia',strtotime($booking->customRequestBooking->customRequest->schedule_time))}}</td>
                                            @endif
                                            <td>{{date('d-M-Y h:ia',strtotime($booking->created_at))}}</td>
                                            <td>{{date('d-M-Y h:ia',strtotime($booking->service_schedule))}}</td>
                                            ...
                                            <td class="text-success">
                                                @foreach ($booking->status_histories as $service_done)
                                                    {{date('d-M-Y h:ia',strtotime($service_done->created_at))}}
                                                @endforeach
                                            </td>
                                            @endif
                                            <td>
