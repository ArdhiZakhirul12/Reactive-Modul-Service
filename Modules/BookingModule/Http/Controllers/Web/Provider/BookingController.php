...
class BookingController extends Controller
{
    ...
    public function details($id, Request $request): View|Factory|RedirectResponse|Application
    {
        ...
        $booking = $this->booking->with(['detail.service' => fn($query) => $query->withTrashed(), 'detail.service.category', 'detail.service.subCategory', 'detail.variation', 'customer', 'provider', 'service_address','reschedule', 'serviceman', 'service_address', 'status_histories.user'])->find($id);
    ...
    public function statusUpdate($bookingId, Request $request): JsonResponse
    {
        ...
        if (isset($booking)) {
            ...
            if ($booking->isDirty('booking_status')) {
                ...
                if ($request['booking_status'] == 'reschedule') {
                    $this->rescheduleBooking($booking, 1);
                }elseif($request['booking_status'] == 'pendingSparepart'){
                    $this->rescheduleBooking($booking, 14);
                }
    ...
    public function servicemanUpdate($bookingId, Request $request): JsonResponse
    {
        ...
        if (isset($booking)) {
            ...
            if ($booking->isDirty('serviceman_id')) {
                ...
                $bookingStatusHistory->booking_id = $bookingId;
                $bookingStatusHistory->changed_by = $request->user()->id;
                $bookingStatusHistory->booking_status = 'deliver seviceman';
                $bookingStatusHistory->save();
                return response()->json(response_formatter(DEFAULT_STATUS_UPDATE_200), 200);
            }
            return response()->json(response_formatter(NO_CHANGES_FOUND), 200);
    ...
    {
        $booking_category = $this->booking->find($id)->category_id;
        $booking = $this->booking->with([
            'detail.service' => function ($query) {
                $query->withTrashed();
            },
            'customer' => function ($q) use ($booking_category) {
                $q->withTrashed()->with(['customer_machine.detail_customer_machine' => function ($query) use ($booking_category) {
                    $query->where('category_id', $booking_category);
                }])->whereHas('customer_machine');
            },
            'subCategory',
            'provider', 'service_address', 'serviceman', 'service_address', 'status_histories.user',
            'status_histories' => function ($q) {
                $q->select('booking_id', 'updated_at')->whereBookingStatus('completed')->latest('updated_at')->first();
            }
        ])
            ->find($id);
        // $booking = $this->booking->with(['detail.service' => function ($query) {
        //     $query->withTrashed();
        // }, 'subCategory', 'customer', 'provider', 'service_address', 'serviceman', 'service_address', 'status_histories.user'])->find($id);
