...
class BookingController extends Controller
{
    ...
    public function statusUpdate(Request $request, string $bookingId): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            ...
            'ongoing_photos' => 'nullable|array',
            'longitute' => 'nullable|string',
            'latitude' => 'nullable|string',
            'serviceman_notes' => 'nullable|string'
        ]);
        ...
        if (isset($booking)) {
            ...
            $ongoing_photos = [];
            if ($request->has('ongoing_photos')) {
                foreach ($request->ongoing_photos as $image) {
                    $ongoing_photos[] = file_uploader('booking/ongoing/', 'png', $image);
                }
            }
            ...
            //     }
            // }
            if ($bookingStatus == 'servicemanDone' && (business_config('service_complete_photo_evidence', 'booking_setup'))?->live_values == 1) {
                if ($request->has('evidence_photos')) {
                    foreach ($request->evidence_photos as $image) {
                        $evidencePhotos[] = file_uploader('booking/evidence/', 'png', $image);
                    }
            ...
            Log::info('Data Eviden Photo Masuk', context:$evidencePhotos);

            if ($request->has('ongoing_photos')) {
                $booking->ongoing_photos = $ongoing_photos;
                Log::info('Data Ongoing Masuk',context:$ongoing_photos);
            }
            if ($request->has('serviceman_notes')) {
                $booking->serviceman_notes = $request->serviceman_notes;
            }
            ...
            if($request->has('status_history')){
                $bookingStatusHistory->booking_status = $request['status_history'];
            }else{
    ...
    public function bookingDetails(Request $request, string $id): JsonResponse
    {
        $booking = $this->booking->with([
            'detail.service', 'schedule_histories.user', 'status_histories.user', 'subCategory' => function($q){$q->withoutGlobalScope('zone_wise_data')->with('detail_customer_machine');}, 'service_address', 'customer', 'provider', 'zone', 'serviceman.user', 'booking_partial_payments'
    ...
    public function bookingList(Request $request): JsonResponse
    {
        ...
        $bookings = $this->booking->with(['customer', 'serviceman.users','subCategory' => function ($q) {
