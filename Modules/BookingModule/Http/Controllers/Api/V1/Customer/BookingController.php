...
class BookingController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $bookings = $this->booking
            ->with(['customer', 'subCategory' => function ($q) {
            ...
            ->when($request['booking_status'] != 'all', function ($query) use ($request) {
                if ($request['booking_status'] == 'accepted') {
                    return $query->ofBookingStatusAccepted();
                } else {
    ...
    public function show(Request $request, string $id): JsonResponse
    {
        ...
        //     'detail.service', 'schedule_histories.user','subCategory', 'status_histories.user', 'service_address', 'customer', 'provider', 'zone', 'serviceman.user', 'booking_partial_payments'
        // ])->where(['id' => $id])->first();
        ...
        $booking = $this->booking->where(['customer_id' => $request->user()->id])->with([
            ...
            'subCategory' => function ($q) { $q->withoutGlobalScope('zone_wise_data')->with('detail_customer_machine');},
    ...
    {
        $validator = Validator::make($request->all(), [
            'booking_status' => 'required|in:canceled,customerAgrees,reschedulingRequest',
        ]);
