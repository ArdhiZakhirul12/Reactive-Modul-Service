...
trait BookingScopes
{
    ...
    public function scopeOfBookingStatus($query, $status): void
    {
        ...
        // Scope untuk menampilkan booking dengan status 'accepted' atau 'reschedule'
        public function scopeOfBookingStatusAccepted($query): void
        {
            $query->whereIn('booking_status', ['accepted', 'reschedule']);
        }
    ...
    public function scopeProviderPendingBookings($query, Provider $provider, $maxBookingAmount): mixed
    {
        if ($provider?->service_availability && (!$provider?->is_suspended || !business_config('suspend_on_exceed_cash_limit_provider', 'provider_config')->live_values)) {
            ...
            // $subscribedSubCategories = SubscribedService::where(['is_subscribed' => 1])->pluck('sub_category_id')->toArray();
// 
            return $query
