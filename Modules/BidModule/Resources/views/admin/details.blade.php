...
@section('content')
        ...
        <div class="row">
            <div class="col-12">
                <div class="d-flex align-items-center gap-3 mb-3">
                    ...
                    <button class="btn btn--danger ms-auto" data-bs-toggle="modal" data-bs-target="#rejectCustomModal" data-toggle="tooltip">
                        <span class="material-symbols-outlined">cancel</span>{{ translate('Batalkan') }}
                    </button>
                    @endif
                </div>

                <div class="modal fade" id="rejectCustomModal" tabindex="-1" aria-labelledby="rejectCustomModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <form action="{{ route('admin.booking.custom-request.reject', ['id' => $post->id]) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="modal-header">
                                    <h5 class="modal-title" id="rejectCustomModalLabel">{{ translate('Batalakan  Permintaan') }}</h5>
                                ...
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ translate('Tutup') }}</button>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="row g-3">
                                    ...
                                    <div class="card-body pb-5">
                                        <div class="media flex-wrap gap-3">
                                            ...
                                            <div class="media-body">
                                                ...
                                                <h5 class="text-primary mb-2">{{$post?->user?->first_name.' '.$post?->user?->last_name}}</h5>
                                                ...
                                                <div class="d-flex align-items-center gap-2">
                                                    ...
                                                    <p>{{Str::limit($post?->address?->address??translate('not_available'), 100)}}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            {{-- @dd($post->status) --}}
                            @if ($post->status == 'Pending')                                                            
                            <form action="{{route('admin.booking.custom-request.store',[$post->id])}}" method="post"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select class="select-identity theme-input-style mb-30" name="provider_id" required>
                                            <option value="{{$providers?->id}}">{{$providers?->company_name}}</option>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select class="select-identity theme-input-style mb-30" id="machine_select" name="sub_category_id" required>
                                            <option value="{{$machines?->id}}">{{$machines?->name}}</option>                                                                                        
                                        </select>
                                    </div>                
                                    {{-- @dd($machines->services) --}}
                                    {{-- <div class="col-md-6">
                                    ...
                                    <div class="col-md-6 d-flex align-items-center">
                                        <div class="me-2 w-100">
                                            <label class="mb-2" for="">{{translate('Service')}}</label>
                                            <select class="select-identity theme-input-style form-control mb-30" id="service_select" name="service_id" required>                                            
                                                <option value="">{{translate('Select_Service')}}</option>
                                                @foreach ($machines->services as $service)                                            
                                                <option value="{{ $service->id }}">{{$service->name}}</option>                                                                                            
                                                @endforeach
                                        ...
                                        <button type="button" onclick="window.location.href='{{ route('admin.service.create') }}'" class=" btn btn--primary d-flex align-items-center">
                                            <span class="material-icons" title="{{ translate('add_new_employee') }}">add</span>
                                        </button>
                                    </div>
                                    <div class="col-md-6">        
                                        <label class="mb-2" for=""> {{translate('Service_Schedule')}} </label>

                                            <input type="datetime-local" class="form-control h-45" name="service_schedule"
                                                value="" id="service_schedule"
                                    ...
                                    {{-- <section>
                                        <div class="d-flex flex-wrap gap-20 mb-3">
                                            ...
                                            <button type="button" class="btn btn--primary" id="service-variation">
    ...
    <script>
        // document.getElementById('machine_select').addEventListener('DOMContentLoaded', async function() {
        ...
        //     const url1 = await `${url}/admin/booking/custom-request`
        ...
        document.getElementById('service_select').addEventListener('change', async function() {
            ...
            if (!serviceId) return;        
            // Kirim permintaan AJAX untuk mendapatkan daftar layanan berdasarkan sub_category_id
            const baseUrl = await `${url}/admin/booking/custom-request`
