...
class ProviderController extends Controller
{
    ...
    private SubscribedService $subscribed_service, $subscribed_sub_categories;
    ...
    $top_categories = $this->booking
        ...
        return view('providermanagement::dashboard', compact('data', 'chart_data','servicemenList'));
    }
    ...
    public function accountInfo(Request $request): View|Factory|RedirectResponse|Application
    {
        ...
        if ($pageType == 'overview') {
            ...
            $provider = $this->provider
                        ->leftJoin('accounts','providers.user_id', '=', 'accounts.user_id')
                        ->leftJoin('bookings','bookings.provider_id','=','providers.id')
                        ->where('providers.user_id',$request->user()->id)
                        ->select(
                        'providers.*',
                        'accounts.user_id',
                        'accounts.balance_pending',
                        'accounts.received_balance',
                        'accounts.account_payable',
                        'accounts.account_receivable',
                        'accounts.total_withdrawn',
                        'accounts.total_expense',
                        DB::raw('count(bookings.provider_id) as booking_count')
                        )
                        ->groupBy('providers.company_name')
                        ->first();
            // \dd($provider);
            $bookingOverview = DB::table('bookings')->where('provider_id', $request->user()->provider->id)
                ->select('booking_status', DB::raw('count(*) as total'))
            ...
            if ($min == 0 && $max == 0) {
                ...
                do {
                    ...
                    //     'num4' => $num4,
                    //     'num5' => $num5,
                    //     'in_array' =>in_array($num5, array($mid, $mid1, $mid2, $num4))
                    // ]);
                } while (in_array($num5, array($mid, $mid1, $mid2, $num4)) && $num5!=0);
