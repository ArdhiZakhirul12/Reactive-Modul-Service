...
class BookingReportController extends Controller
{
    ...
    public function getBookingReportDownload(Request $request): string|StreamedResponse
    {
        ...
        $filteredBookings = self::filterQuery($this->booking, $request)
            ...
            // $ongoing = $filteredBookings->first()->status_histories->where('booking_status', 'ongoing')->first();
            // $ongoing = $filteredBookings->first()->status_histories;
            // $completed = $filteredBookings->first()->status_histories->where('booking_status', 'completed')->first();
            // // dd($ongoing,$completed,$filteredBookings);
            // $combinedBookings = $filteredBookings->map(function ($booking) use ($ongoing, $completed ) {
            //     $booking->ongoing = Carbon::parse($ongoing->created_at)->format('Y-m-d H:i:s');
            //     $booking->completed = Carbon::parse($completed->created_at)->format('Y-m-d H:i:s');
            //     return $booking;
            // });
        ...
        return (new FastExcel($filteredBookings))->download(time().'-booking-report.xlsx', function ($booking) {
            return [
                ...
                'Nama Customer' => isset($booking->customer) ? ($booking->customer->first_name . ' ' . $booking->customer->last_name) : '',
                'No Hp Customer' => isset($booking->customer) ? ($booking->customer->phone??'') : '',
                'Email Customer' => isset($booking->customer) ? ($booking->customer->email??'') : '',
                'Nama Teknisi' => isset($booking->serviceman) ? ($booking->serviceman->user->first_name . ' ' . $booking->serviceman->user->last_name) : '',
                // 'Servis Mulai' => isset($booking->ongoing) ? ($booking->ongoing) : '',
                // 'Servis Selesai' => isset($booking->completed) ? ($booking->completed) : '',
                'Mesin' => isset($booking->subCategory) ? ($booking->subCategory->name) : '',
                'variant' => implode(', ', $booking->detail->pluck('variant_key')->toArray()),                
