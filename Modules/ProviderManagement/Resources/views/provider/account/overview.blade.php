...
@section('content')
    ...
    <div class="main-content">
        <div class="container-fluid">
            @if(provider_warning_amount_calculate($provider->account_payable,$provider->account_receivable) == '80_percent'
            ...
            <div class="card mt-4">
                <div class="card-body">
                    <div class="row gy-2">
                        @php($checkAdjust = $provider->account_payable == $provider->account_receivable && $provider->account_payable == 0 && $provider->account_receivable == 0)
                        ...
                        <div class="@if($checkAdjust) col-lg-6 @else col-lg-3 @endif">
                            <div class="statistics-card statistics-card__style2">
                                <h2>{{with_currency_symbol($provider->account_receivable)}}</h2>
                        ...
                        <div class="@if($checkAdjust) col-lg-6 @else col-lg-3 @endif">
                            <div class="statistics-card statistics-card__style2">
                                <h2>{{with_currency_symbol($provider->account_payable)}}</h2>
                        ...
                        @if($provider->account_payable > $provider->account_receivable && $provider->account_receivable != 0)
                        ...
                        @elseif($provider->account_payable > $provider->account_receivable && $provider->account_receivable == 0)
                        {{-- @elseif($provider->owner->account->account_payable > $provider->owner->account->account_receivable && $provider->owner->account->account_receivable == 0) --}}
                            <div class="col-lg-6">
                                <div
                                    class="statistics-card statistics-card__style2 d-flex justify-content-between gap-2 align-items-center">
                                    <div class="">
                                        <h2>{{with_currency_symbol($provider->account_payable - $provider->account_receivable)}}</h2>
                                        {{-- <h2>{{with_currency_symbol($provider->owner->account->account_payable - $provider->owner->account->account_receivable)}}</h2> --}}
                                        <p class="d-flex align-items-center gap-2 text-muted">{{translate('Payable Balance')}}
                        ...
                        @elseif($provider->account_receivable > $provider->account_payable && $provider->account_payable != 0)
                        {{-- @elseif($provider->owner->account->account_receivable > $provider->owner->account->account_payable && $provider->owner->account->account_payable != 0) --}}
                            <div class="col-lg-6">
                                <div
                                    class="statistics-card statistics-card__style2 d-flex justify-content-between gap-2 align-items-center">
                                    <div class="">
                                        <h2>{{with_currency_symbol($provider->account_receivable - $provider->account_payable)}}</h2>
                        ...
                        @elseif($provider->account_receivable > 0 && $provider->account_payable == 0)
                        ...
                        @elseif($provider->account_payable == $provider->account_receivable && $provider->account_payable != 0 && $provider->account_receivable != 0)
                    ...
                    <div class="row gy-2">
                        <div class="col-md-4">
                            <div class="p-4 rounded pending_withdraw">
                                ...
                                <h5 class="d-flex align-items-center gap-2 text-muted">{{with_currency_symbol($provider->balance_pending)}}
                        ...
                        <div class="col-md-4">
                            <div class="p-4 rounded already_withdraw">
                                ...
                                <h5 class="d-flex align-items-center gap-2 text-muted">{{with_currency_symbol($provider->total_withdrawn)}}
                    ...
                    <div class="bg-white information-details-box">
                        <div class="row g-4">
                            <div class="col-lg-3">
                                ...
                                <p><strong
                                        class="text-capitalize">{{translate($provider->identification_type)}}
                                        -</strong> {{$provider->identification_number}}</p>
                                {{-- <p><strong
                            ...
                            <div class="col-lg-9">
                                <div class="d-flex flex-wrap gap-3 justify-content-lg-end">
                                    @if(isset($provider->identification_image) && count($provider->identification_image) > 0)
                                        @foreach($provider->identification_image as $key=>$image)
                                            <div>
