...
class CartController extends Controller
{
    ...
    {
        // return response()->json('tes dari kho+da');
        ...
        Log::info('sudah buat validator');

        if ($validator->fails()) {
            return response()->json(response_formatter(DEFAULT_400, null, error_processor($validator)), 400);
        }
        Log::info('sudah validator');

        $customerUserId = $this->customerUserId;
        Log::debug(message:'sudah inisialisaisi customer id',context:[$customerUserId]);

        // dd($customerUserId);
        $this->addedToCartUpdate($customerUserId, $request['service_id'], !$this->isCustomerLoggedIn);
        Log::info('sudah addedToCartUpdate');
        ...
        $variation = $this->variation->withoutGlobalScope('zone_wise_data')
            ->where(['service_id' => $request['service_id']])
            ->where(['variant_key' => $request['variant_key']])
            ->first();
        // dd($request->all(), $variation);
        Log::debug(message: 'sudah inisialisaisi variation', context: [$variation]);

        if (isset($variation)) {
            Log::info('sudah pengujian variation');
            // $service = $this->service->find($request['service_id']);
            $service = $this->service->withoutGlobalScope('zone_wise_data')->find($request->service_id);
            Log::debug(message: 'sudah inisialisaisi service', context: [$service, $request['service_id']]);
            ...
            Log::debug(message: 'sudah inisialisaisi checkCart', context: [$checkCart]);

            $cart = $checkCart ?? $this->cart;
            Log::debug(message: 'sudah inisialisaisi cart', context: [$cart]);
            $quantity = $request['quantity'];
            Log::debug(message: 'sudah inisialisaisi quantity', context: [$quantity]);

            $basicDiscount = basic_discount_calculation($service, $variation->price * $quantity);
            Log::debug(message: 'sudah inisialisaisi basicDiscount', context: [$basicDiscount]);
            $campaignDiscount = campaign_discount_calculation($service, $variation->price * $quantity);
            Log::debug(message: 'sudah inisialisaisi campaignDiscount', context: [$campaignDiscount]);
            $subtotal = round($variation->price * $quantity, 2);
            Log::debug(message: 'sudah inisialisaisi subtotal', context: [$subtotal]);

            $applicableDiscount = ($campaignDiscount >= $basicDiscount) ? $campaignDiscount : $basicDiscount;
            Log::debug(message: 'sudah inisialisaisi applicableDiscount', context: [$applicableDiscount]);

            $tax = round((($variation->price * $quantity - $applicableDiscount) * $service['tax']) / 100, 2);
            Log::debug(message: 'sudah inisialisaisi tax', context: [$tax]);
            ...
            if (!$this->isCustomerLoggedIn) {
                Log::info('sudah pengujian customer login');
                $guest = $this->guest;
                $guest->ip_address = $request->ip();
                $guest->guest_id = $request->guest_id;
                $guest->save();
                Log::debug(message: 'sudah inisialisaisi guest', context: [$guest]);
            }
            Log::info('keluar pengujian customer login');
    ...
    public function list(Request $request): JsonResponse
    {
        ...
        $customerUserId = $this->customerUserId;
        $cart = $this->cart
            ->with(['customer', 'provider.owner', 'category', 'sub_category' => function($q){
            $q->withoutGlobalScope('zone_wise_data')->with('detail_customer_machine');
            }, 'service' => function($q){
