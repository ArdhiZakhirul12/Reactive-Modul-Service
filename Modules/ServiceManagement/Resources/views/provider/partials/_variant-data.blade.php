...
@if(session()->has('variations'))
    @foreach(session('variations') as $key=>$item)
        <tr>
            ...
            {{-- <td>
                <input type="hidden" value="{{$item['price']}}" class="theme-input-style" id="default-set-{{$key}}"
                       onkeyup="set_values('{{$key}}')" step="any">
            </td> --}}
            {{-- @foreach($zones as $zone)
                <td>
                    <input type="hidden" name="{{$item['variant_key']}}_{{$zone->id}}_price" value="{{$item['price']}}"
            ...
            <td>
                <a class="btn btn--danger service-ajax-remove-variant"
                   ...
                   data-route="{{route('provider.service.ajax-remove-variant',[$item['variant_key']])}}">
