...
class ServiceController extends Controller
{
    ...
    public function index(Request $request): View|Factory|Application
    {
        ...
        $services = $this->service->with(['category.zonesBasicInfo','subCategory.zonesBasicInfo'])->latest()
            ->when($request->has('search'), function ($query) use ($request) {
                $keys = explode(' ', $request['search']);
                foreach ($keys as $key) {
                // $query->orWhere('name', 'LIKE', '%' . $key . '%');
                $query->orWhere('name', 'LIKE', '%' . $key . '%')
                ->orWhereHas('subCategory', function ($subQuery) use ($key) {
    ...
    { 
        Log::info(message:'==========================================');
        Log::info(message:'Buat Service');
        Log::debug(message:'request',context:$request->all());
        $variations = session('variations');
        Log::debug(message:'Variasi',context:[$variations]);
        ...
        Log::debug(message:'Tags', context:['tagid' => $tagIds]);

        $parent = $this->category->where('position',1)->first()->id;        
        Log::debug(message:'Machine Category', context:[$parent]);
        ...
        Log::debug(message:'Service', context:[$service]);
        ...
        // if ($variations) {
            ...
            // }
        // }
        Log::debug(message:'Variasi',context:[$variationFormat,$variations,]);
